...
os.chdir('/usr/tmp/Query_API_v1/output_files')
...
receiver_address = "esteban.estrada@vtr.cl"
#Setup the MIME
...
attach_file_name = ("report_250520.zip")
attach_file = open(attach_file_name, 'rb') # Open the file as binary mode
payload = MIMEBase('application', 'zip') #'octate-stream')
payload.set_payload((attach_file).read())
encoders.encode_base64(payload) #encode the attachment
#add payload header with filename
payload.add_header('Content-Decomposition', 'attachment', filename=attach_file_name + '.zip')
